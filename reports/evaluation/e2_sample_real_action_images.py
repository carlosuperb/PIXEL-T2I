...
This script randomly samples a subset of real action sprite sheets from the
processed action dataset and copies them into a dedicated evaluation directory.

The sampled images are used as the reference "real" distribution when computing
evaluation metrics (e.g., FID) for image-conditioned action generation.
"""
...
# source dataset (processed action sprite sheets)
SOURCE_DIR = Path("pixel_character_dataset/processed/dataset_actions/images")

# target directory for evaluation (real action images)
TARGET_DIR = Path("reports/evaluation/real_action")
...
def main():
    """
    Sample a fixed number of action sprite sheets from the dataset and copy them
    ...
    print(f"Sampling {NUM_SAMPLES} action images...")
